...
function ProgressionBar() {
  ...
  return (
    <div className="progression-div">
      <p className="progression-info">Goal: {format(settings.finalGoal)} - <span className="time">{formatLeftTime(timeLeft)}</span>{player.everMadeRun && (<span>&emsp;|&emsp;Best run: {player.bestRun === null ? 'no' : formatTime(player.bestRun)} - <span className="effect">Effect: {format(player.runEffect)}x</span>&emsp;|&emsp;Best points: {format(player.bestPointsOfRun)} - <span className="effect">Effect: {format(player.bestPointsOfRunEffect)}x</span></span>)}</p>
