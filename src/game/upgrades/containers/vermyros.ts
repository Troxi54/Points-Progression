import { everPerformed } from "@/game/resetLayers/utils/get";
import { createUpgradeDataContainer } from "@/game/upgrades/utils/create";
import { hasUpgrade } from "@/game/upgrades/utils/has";

const vermyrosUpgrades = createUpgradeDataContainer({
  id: "vermyros",
  usePlayer: ({ mergedPlayer }) => ({
    everMadeNullith: everPerformed(mergedPlayer, "nullith")
  }),
  forceShow: ({ player }) => everPerformed(player, "nullith"),
  spendCurrency: ({ player }) => !hasUpgrade(player, "nullith", 1),
  upgrades: [
    {
      id: "vermyros_1",
      description:
        "Unlocks Vermyte Upgrade and prevents Reset and Tier upgrades from spending Points",
      currency: "vermytes",
      cost: 1
    },
    {
      id: "vermyros_2",
      description: `Enables passive Tier Reset generation, replaces Auto Tier toggle with Auto Tier Up toggle, and keeps 1st and 2nd Reset and Tier upgrades`,
      cost: 1e90
    },
    {
      id: "vermyros_3",
      description: "Keeps 3-6 Tier upgrades",
      cost: 1e96
    },
    {
      id: "vermyros_4",
      description:
        "Prevents Tier from resetting anything, removes Auto Tier Up toggle, and generates Vermytes equal to 0.1% of your Best Vermytes per second",
      cost: 1e114
    },
    {
      id: "vermyros_5",
      description:
        "Unlocks Amplivault and generates Vermytes equal to 1% of your Best Vermytes per second",
      cost: 1e133
    },
    {
      id: "vermyros_6",
      description:
        "Generates Vermytes equal to 10% of your Best Vermytes per second",
      cost: 1e162
    },
    {
      id: "vermyros_7",
      description:
        "Generates Vermytes equal to 100% of your Best Vermytes per second",
      cost: 1e186
    },
    {
      id: "vermyros_8",
      description:
        "Unlocks Energy, automatically updates Best Vermytes, and removes Auto Vermyros toggle",
      cost: 1e212
    },
    {
      id: "vermyros_9",
      description: "Prevents Vermyte Upgrade from spending Vermytes",
      cost: 1e230
    },
    {
      id: "vermyros_10",
      description:
        "Permanently unlocks Dark Energy and automates Vermyte Upgrade",
      cost: 1e250
    }
  ]
});

export default vermyrosUpgrades;
